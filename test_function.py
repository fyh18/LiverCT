...
def LabelPredict(test_set, is_cnt, select_feature, 
                 ...
                 skip_lv1=False, lv1_label=None, save_to=None):
    """
    :param test_set:
    :param is_cnt:
    :param save_path:
    ...
    if select_feature:
        gene_selected = pd.read_csv(model_path + "/saves/" + model_name + "/train_gene_list.csv", index_col=0)
    ...
    if skip_lv1:
        ...
        if select_feature:
            gene_selected_lv1 = pd.read_csv(model_path + "/saves/" + model_name + "/gene_selected.csv", index_col=0)
            gene_selected_lv1 = list(gene_selected_lv1.values.T[0])
        else:
            gene_selected_lv1 = gene_selected
        clf = joblib.load(model_path + "/saves/" + model_name + "/mlp_clf_lv1.pkl")
    ...
    adata_list = []
    celltype_tree = np.load(model_path + "/saves/" + model_name + "/celltype_tree.npy", allow_pickle='TRUE').item()
    file_list = np.array(os.listdir (model_path + "/saves/" + model_name + '/'))
    for g_label in np.unique(test_adata.obs['pred_lv1']):
        lv2_celltypes = celltype_tree[g_label]
        start = datetime.datetime.now()
        print(start, g_label, "lv2 predict...")
        if os.path.exists(model_path + "/saves/" + model_name + '/voting_clf_lv2_' + g_label + '.pkl'):
            if select_feature:
                gene_selected_lv2 = pd.read_csv(model_path + "/saves/" + model_name + "/gene_selected_" + g_label + ".csv",
                                                index_col=0)
            ...
            clf2 = joblib.load(model_path + "/saves/" + model_name + '/voting_clf_lv2_' + g_label + '.pkl')
            ...
            if transition_predict:
                ...
                for file in file_list_ovo:
                    ...
                    if sum(cell_slice) > 0:
                        ovo_clf = joblib.load(model_path + "/saves/" + model_name + '/' + file)
                        #if select_feature:
                        #    gene_selected_ovo = pd.read_csv(model_path + "/saves/" + model_name +
                        #                               "/gene_selected_" + lv2_label1 + lv2_label2 + ".csv", index_col=0)
                        #    gene_selected_ovo = list(gene_selected_ovo.values.T[0])
                        #else:
                        #    gene_selected_ovo = gene_selected
            ...
            adata_part.obs["voting_lv2_2"] = ["None"] * adata_part.n_obs
        ...
        if disease_predict:
            ...
            for lv2_label in lv2_celltypes:
                ...
                if sum(cell_slice) > 0:
                    ...
                    oc_clf = joblib.load(model_path + "/saves/" + model_name + '/' + file)
    ...
    # annotation type
    test_adata.obs["transition"] = ["transition" if i < 0.3 else "normal" for i in test_adata.obs[ 'transition_distance']]
    test_adata.obs["disease"] = ["disease" if i < -0.3 else "normal" for i in test_adata.obs[ 'disease_distance']]
    unknown_list = test_adata.obs['suggest_label_lv1'].isnull()
    unknown_list[test_adata.obs['suggest_label_lv1'] == 'UnknownEpithelial'] = True
    test_adata.obs['unknown'] = ['unknown' if i else 'normal' for i in unknown_list]
    
    annot_type = np.array(test_adata.obs['disease'])
    annot_type[test_adata.obs['transition'] == 'transition'] = 'transition'
    annot_type[test_adata.obs['unknown'] == 'unknown'] = 'unknown'
    test_adata.obs['annot_type'] = list(annot_type)
    
    # exp score
    test_adata.obs["transition_score"] = np.exp(-test_adata.obs['distance_ovo'])
    # disease score
    test_adata.obs["disease_score"] = -test_adata.obs['disease_distance']
    
    df_groups = [test_adata.obs["suggest_label_lineage"], test_adata.obs["suggest_label_lv1"],
                 test_adata.obs["suggest_label_lv2"], test_adata.obs["pred_lv1"], 
                 test_adata.obs["voting_lv2_1"], test_adata.obs["pred_lv2_ovo"], 
                 test_adata.obs["disease_distance"], test_adata.obs['transition_score'], 
                 test_adata.obs["annot_type"]]
    softvote_pred = reduce(lambda left, right: pd.concat([left, right], axis=1), df_groups)
    
    # if save_to is not None:
    #     softvote_pred.to_csv(save_to + "/" + model_name + "_softvote_res.csv")

    return test_adata.obs
